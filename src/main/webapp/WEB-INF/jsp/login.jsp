			...
			<table border="1" style="width: 500px;">
				<tr>
					<th><div class="blank"
							...
							<h4>
								Healthy Life（症状検索システム）<br>会員ログインへめんそ～れ
							</h4>
						</div></th>
				</tr>
				<tr>
					<td><div class="blank" style="padding: 20px">
							...
							<c:if test="${not empty msg}">
								<c:if test="${msg =='ログアウトしました'}">
									<div class="msg"
										style="padding: 20px; background-color: #CCFF33;">
										<p>${msg}</p>
									</div>
								</c:if>
								<c:if test="${msg !='ログアウトしました'}">
									<div class="msg"
										style="padding: 20px; background-color: #FFCCCC;">
										<p>${msg}</p>
									</div>
								</c:if>
								<p></p>
							...
							<form action="/healthylife/Login" method="post">
								E-mailアドレス<br>
								<input type="text" name="email" style="width: 460px"
									value="<c:out value="${loginerrUser.email}" />">

								<p></p>
								パスワード<br>
								<input type="password" name="pass" style="width: 460px"
									value="<c:out value="${loginerrUser.pass}" />">
